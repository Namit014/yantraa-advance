...
function FlowCanvas({ currentQuery, designData }: { currentQuery?: string; designData?: any }) {
  ...
  const [isERCing, setIsERCing] = useState(false);
  const handleRunERC = useCallback(async () => {
    try {
      setIsERCing(true);
      setErcReport("Running Electrical Rules Check (ERC)...");
      const res = await fetch("http://localhost:8000/api/skidl/erc", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ nodes: storeNodes, edges: storeEdges }),
      });
      const data = await res.json();
      setErcReport(data.report || "No report generated.");
    } catch (e) {
      setErcReport("Failed to run ERC. Is the backend running?");
    } finally {
      setIsERCing(false);
    }
  }, [storeNodes, storeEdges, setErcReport]);
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
        ...
        >
          ...
          <button
            ...
            disabled={isERCing || storeNodes.length === 0}
            style={{
              display: "flex",
              alignItems: "center",
              gap: 7,
              ...
              borderRadius: 8,
              border: "1px solid",
              borderColor: isERCing || storeNodes.length === 0 ? "#1a2744" : "#10b981",
              backgroundColor: isERCing || storeNodes.length === 0 ? "#0d1528" : "#064e3b",
              color: isERCing || storeNodes.length === 0 ? "#374151" : "#6ee7b7",
              fontSize: 12,
              fontFamily: "monospace",
              fontWeight: 700,
              cursor: isERCing || storeNodes.length === 0 ? "not-allowed" : "pointer",
              transition: "all 0.2s",
              whiteSpace: "nowrap",
            }}
            onMouseEnter={(e) => {
              if (!isERCing && storeNodes.length > 0) {
          ...
          >
            {isERCing ? <Loader2 size={13} className="animate-spin" /> : <AlertCircle size={13} />}
            {isERCing ? "Running..." : "Run ERC"}
          </button>
